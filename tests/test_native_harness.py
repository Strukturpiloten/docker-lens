...
class NativeHarnessTests(unittest.TestCase):
    def test_created_resources_are_removed_even_when_create_reports_failure(self) -> None:
        for fault in ("volume", "pull", "run", "run_exists_error", "pull_exists_error", "preflight_exists_error"):
...
case "$command" in
  ...
  run)
    ...
    [[ $FAKE_NATIVE_FAULT == run || $FAKE_NATIVE_FAULT == run_exists_error ]] && exit 42
    echo fake-id ;;
  inspect)
    if [[ $* == *Labels* ]]; then
...
""",
                ...
                result = subprocess.run(
                    ["bash", str(ROOT / "scripts/native-conformance.sh"), "upstream-rootful"],
                    env=env,
                    capture_output=True,
                    text=True,
                    timeout=15,
